...
csv_files <- list.files(path = "C:/Datanew", pattern = "*.csv", full.names = TRUE)

# Read each CSV file into a data frame and store in a list
experiment_list <- csv_files %>%
  map(~ read_csv(.x) %>% as_tbl_obs())

# Combine all data frames into one data frame
Experiment <- bind_rows(experiment_list)

# Read Stimuli and Conditions data
Stimuli <- read_csv("C:/Users/fbend/Downloads/YET-main/YET-main/yeta/1 - Kopie/Stimuli/Stimuli.csv") %>% as_tbl_obs()
Conditions <- read_csv("C:/Users/fbend/Downloads/YET-main/YET-main/yeta/1 - Kopie/Conditions.csv") %>% as_tbl_obs()
...
final_data <- final_data %>%
  mutate(
    angles = map2(x.x, y.x, ~calculate_visual_angle_center(.x, .y, screen_center_x, screen_center_y, screen_width_cm, screen_height_cm, screen_res_x, screen_res_y, viewing_distance_cm)),
...
final_data <- final_data %>%
  rowwise() %>%
  mutate(
    distance_px = calculate_distance_px(x.x, y.x, x.y, y.y),
