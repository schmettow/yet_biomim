...
author: "M Schmettow, _____, S. Borsci"
format: docx
editor: source
...
It is highly unlikely that such a specialized receiver architecture evolved without there being a sender. In contrast to the sender, it is also unlikely that it evolved as a one-point mutation. A possibility is that the white sclera encountered a rudimentary precursor of the EDD. As @Laidre2013 points out, it is a common assumption that "many signals have evolved from what once were cues." 

For the white sclera to become a cue, it had to had possessed visual properties that made it salient for receivers that already existed, which converges with the idea that scleral pigmentation evolved as a camouflage.

Indeed, the high contrast between the white sclera and the dark pupil creates a visually striking pattern that seems to draw "natural" attention. This salience would have made it easier for early humans to detect and interpret eye movements, even in peripheral vision or at a distance.
...
However, measuring computational simplicity on biological observations bares the risk of circular reasoning, because ease of processing is a consequence of natural selection.
In our first study, we test computational simplicity of the white sclera by constructing a technical receiver device using minimal sensory and computational resources. If the minimalist *eye tracking* device is accurate enough to provide benefits, an upper limit for computational complexity of the signal can be established.
...
If the CEH, extended by the receiver mechanism, is correct, the human eye should be computationally simple to read with visual cues available to a human observer.
Most commercial eye-tracking devices use signals that are not available to the human visual system, such as infrared reflections or magnetic fields (@Singh2012). 

The more recent video-oculographic devices instead use visible light cameras (@Zhou2004) and recently gave rise to the development of webcam, smartphone and VR eye tracking (@Ciesla2012). Several algorithms have been developed in this line, often making use of advanced machine learning or computer vision techniques (@Krafka2016), which often resemble biological visual processing (@Kumar2017 CHECK).
Video-oculographic devices are the only *biologically plausible* option, because only they work with visible light.  Yet, the involved algorithms are computationally complex and demanding, such as filtering, edge detection or neural networks.
...
The visual cortex can approximately be modeled as a deep neural network and one strategy to a minimum complexity is to find the smallest network that still performs well. In this study we employed the opposite strategy, starting from the most simple device imagineable. 
...
The effectiveness of the Quadbright method was measured as degree errors between real target position and measured position.
From the original experiment only the conditions with a white calibration background was used, resulting in `r nrow(FB24_0)` observations from 24 participants. In order to remove movement artifacts the first and last 500ms where removed and to make the data set more amenable the average over four consecutive frames was taken, resulting in `r nrow(FB24_2)` observations.
...
![Quadbright condition](Illustrations/Exp_QB.png){#fig-qb-experimental}
Example stimuli image with eye direction at 3 o'clock position [@Hoelter2025, with permission]
...
F_stw_2 <- deviation ~ 0 + 
                    ...
                    (0 + 
                      ...
                      exp_600 + 
                      exp_1000|Part) +
                    (0 + Control_70 + exp_140 + qbr_140| Stim)

#m_5 <- lme4::lmer(F_stw_2, data = D_2)
#exp(summary(m_5)$coefficients)
#summary(m_5)

M_5_stw <-rstanarm::stan_glmer( F_stw_2, 
...
The high accuracy even made us wonder whether the receiver in modern humans is similar to a Quadbright device, which gave rise to the second experiment.
The results of the second experiment clearly show that this is not the case. Another receiver clearly exists, which is quite accurate, makes use of more advanced visual features, is extremely fast and varies little between participants.
However, participants were still able to extract Quadbright glance directions at a lower accuracy most of the time. This mechanism requires longer exposure and varies dramatically between individuals.
...
Overall, the Quadbright device showed good accuracy, in the same range as human observers [@Gale2000].
A linear decoder—mathematically equivalent to linear regression—operates in 
$O(d)$ parameter space and performs inference in $O(d)$ time, making it one of the simplest possible mappings from sensory input to behavioral output.
...
It is important to note that the white sclera never evolved as a signal in vertebrates, but as a protection layer around the lense. Only, when the first vertebrates appeared on land, the sclera evolved into a ball, accommodating for the higher refractive index on the cornea-air surface [@AnimalEyes, ch. 9]. The second defining feature, the dark iris and pupil have likewise evolved for biophysical reasons, as a variable aperture to adjust the incoming light intensity to the dynamic range of the retina [@Kruijt2011].

Still, other features of the human eye morphology may have co-evolved with the receiver mechanism. From a computational perspective, it is surprising that the human eyeball produces highly linear gradients, as spinning spherical objects generally produce sinusoidal signals. It is possible that other features have evolved to improve the linearity of the response, such as the high ratio of exposed sclera [@Kobayashi2001]  or the shape of the eye lids. A testable consequence of this idea is that Quadbright devices are less effective for eye direction reading on other primate species, supposed they had a white sclera.
...
By construction we have shown that a simple linear decoder suffices to read glance directions from depigmented eyeballs with astonishing accuracy. 
This supports the idea that a primitive receiver could have existed prior to the evolution of white sclera. However, our results clearly show that human eyeball reading is is not a Quadbright device. A more complex receiver exists, which is extremely fast and accurate.
