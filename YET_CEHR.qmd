...
M_4 <- stan_glmer(deviation ~ Condition * Exposure + 
                  ...
                  family = neg_binomial_2(), data = JGJH,
                  chains = 8)
...
ranef(M_3, mean.func = exp) |> 
  filter(re_factor == "Stim", fixef == "Intercept") |> 
  ggplot(aes(x = as.integer(re_entity), y = center)) +
  geom_point() +
  geom_smooth(se = F)
...
The Cooperative Eye Hypothesis states that the eyeball has evolved to be a computationally efficient sender of glance direction signals. The first study shows that a rather simple machine learning algorithm, the QuadBright algorithm, can estimate eye positions with a reasonable accuracy. As it uses highly degraded images, it can also function under conditions similar to human glance perception, especially glance reading from a distance. This supports the idea that the human eyeball has evolved as a robust and computationally efficient signalling device.

The second study shows that human glance reading is possible with the same information as the QuadBright algorithm uses, albeit with a moderate loss in performance. Exposure times down to 400ms did not cause any further performance loss in both conditions, but with 140ms performance is slightly reduced in both conditions. At 70ms exposure times, performance deteriorates further with degraded images, as compared to the original images.
