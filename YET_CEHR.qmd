...
F_stw <- deviation ~ 0 + exp_70 + qbr_70 + 
                      stp_140 + qbr_140 +
                      stp_400 + qbr_400 +
                      stp_600 + qbr_600 +
                      stp_1000 + qbr_1000 + 
...
```{r}
#| label: fig-fixef-M4
#| fig-cap: "Estimated degree error by condition and exposure time with 95% credibility intervals."
...
As @fig-fixef-M4 shows the degree error is consistently higher in the Quadbright condition. This is most pronounced at the shortest exposure time of 70 milliseconds, with 16.1° in the control condition, and around double the error in the Quadbright condition [@tbl-fixef-M4]. These estimates are fairly uncertain, but still stand out.

With 140ms the error rate show a conditional  effect, where Control remains stable (98.7%), but Quadbright errors drop to 77.4% relative to Control, with high certainty that the effect is real. From this point on, the Quadbright error rates follow the same trend as Control (@fig-fixef-M4). There seem to be two small decreases towards 400ms (88.9%) and 1000ms (92.3%), but these cannot be said with 95% certainty.
...
An implicit assumption is that the receiver mechanism is innate and as such universal in humans. Figure @fig-participant-trajectories shows the participant-level performance trajectories. Initially, all of them showed lower performance in the Quadbright condition. A dominant effect is the enormous participant-level variance in all Quadbright conditions. Initially, the sample appears to fall into two clusters, one in the region below 40 degree errors, and a few participants dramatically failing with Quadbright signals. Starting with 140ms a third group appears with almost stable performance around 10 degree.
...
The Cooperative Eye Theory implies that the eyeball is a very effective sender of glance direction signals, as simple receiver mechanisms are more likely to evolve. By construction of a simple eye tracking device, we tried to establish a limit on how effective the sender is.
Overall, the Quadbright device showed accuracy in the same range as human observers [@Gale2000]. Also the eccentricity effect is similar. 

By the method of ordinary least square, all calculations are based on matrix multiplication and have closed solutions, requiring no costly recursive or iterative procedures. Taking measures 
From a biological perspective, every organism that can sense horizontal and vertical brightness changes, can evolve this function, or certainly acquire it, e.g. by operant learning.

Quadbright is extremely efficient in memory consumption at all steps. The lowest demands are at the input buffer (4 byte) and the highest during the training phase, in which the data buffer holds the coordinates and Quadbright value of nine calibration targets and the covariance matrices for the estimation. In operation, only the current Quadbright value and the four coefficients are needed.

But, has the eye evolved to be such an effective sender of direction signals? The active process of pigmentation implies that this is not so, rather is the white sclera a default condition that comes with liabilities.
The spherical form of eye balls has evolved not for signal efficiency, but for the physical requirements of creating a moving, variable-focus lens-sensor system from tissue and a pressurized liquid. Also the contrasting iris exists to adjust the opening of the lens under varying light levels and the dark pigmentation reduces internal reflections, which produce stray light effects, resulting in reduced contrast levels [@Kruijt2011]. For the same reason photographic and astronomical devices systems use black paint or velour tissue.
...
The simplicity of the Quadbright device supports the idea of sclera camouflage as a default condition among apes, despite the metabolic effort. It is hardly conceivable that an apex predators  would not evolve the ability to read the eyes of their prey. 
Another scenario is that animals equipped with face detection and sufficiently large brains are likely to *learn by experience* that an attempt to flee is often preceded by brightness changes in the eye region.

Is the Quadbright algorithm equivalent with human glance reading? Is input organized as an image of four areas and the visual system performs a series of simple linear equations? Surprisingly good designs emerge from evolutionary processes, 

If Quadbright is a good model of human glance perception, then all humans should be able to read glance directions from brightness four grey areas.
Our second study showed that Quadbright degradation led to a strong overall decline in reading performance. Quadbright is definitely not the mechanism that drives modern human eye direction reading.

While most participants were less accurate with degraded images, the majority of the participants was still much beyond guessing. Crucially, the Quadbright degradation makes it almost impossible for any mechanism that goes beyond brightness gradients, such as edge detection. We can conclude that a minimum of two processing modes for eye direction reading exists. The first one is based on brightness gradients with moderate accuracy, and the second one extracts higher-level features for high accuracy.

A multiple receiver model has been proposed by @Anderson2016, who argue that there may be different mechanisms for precise gaze triangulation and coarse judgement of direction. By using a contrast reversal manipulation they showed that the first depends on geometric cues, whereas the second depends on luminance and motion cues. 

The multiple receiver model is also inline with the evolutionary process where a rudimentary cue receiver paves the way for the co-evolution of signals and more advanced receivers. Brightness gradient processing is not only a good candidate for such a proto-receiver, and there is no reason why this ability should vanish, especially when it still fulfills functions that the more advanced receiver cannot, such as long distance reading and peripheral change detection.
